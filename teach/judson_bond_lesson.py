"""Producer: the actual Judson / James Bond lesson (teach-8xw.15).

Formerly `teach/dnf_bond_lesson.py`. Renamed and re-derived under
teach-8xw.56, when the abstract-algebra test case moved from Dummit & Foote
to Thomas W. Judson's *Abstract Algebra: Theory and Applications* (open,
GNU FDL 1.3+) -- see `teach/judson_algebra_graph.py`'s module docstring for
why, and for the re-derived edge structure this lesson's traversal now walks.
...
someone already extracted, `teach.judson_algebra_graph` is a validated graph
nobody has taught from. This module is the first thing that assembles them
into one real lesson for the epic's actual test case: "teach [the
abstract-algebra text] to a learner whose stated interest is James Bond,"
ending at the graph's `TARGET_NODE_ID` (Lagrange's Theorem).
...
`teach-8xw.15`'s own acceptance test (see `teach/judson_bond_integration.py`)
...
REUSE, NOT REIMPLEMENTATION
  ...
  - `teach.judson_algebra_graph.load_judson_algebra_graph` / `prerequisite_closure`
    -- the real, already-validated Judson chain. No new nodes or edges here.
  ...
  - The closing encouragement line is written to satisfy
    ...
    claimed next step (normal subgroups) is the ordinary next node past the
    target in the graph itself, not an invented reach. This is the
...
Every beat's spy narration is built to still contain the real Judson
...
metaphor and never stated "the number of elements in H must divide the
number of elements in G" would be exactly the failure mode sandbox-prompt.md
warns about: fluent and engaging with nothing underneath for a blind checker
to verify.
"""
...
from teach.judson_algebra_graph import TARGET_NODE_ID, load_judson_algebra_graph, prerequisite_closure
...
_BOND_NARRATION: dict[str, str] = {
    "judson:1.2-sets-and-equivalence-relations": (
        ...
        "Sets and Equivalence Relations starts with assigning each asset "
        "to exactly one handler: that assignment is a function from the "
        "domain of assets to the range of handlers. When no two assets "
        "share a handler, the assignment is one-to-one; when every "
        "handler has at least one asset reporting to them, it's onto; "
        "when both hold at once, it's bijective, and the whole roster is "
    ...
    "judson:3.2-definitions-and-examples": (
        ...
        "in the same repertoire. Definitions and Examples of Groups is "
        "the rulebook that makes that repertoire a group -- the "
    ...
    "judson:3.3-subgroups": (
        ...
        "Subgroups is how Bond vets whether a smaller unit, Q Branch, "
        "sitting inside the network G, can stand on its own: Q Branch is "
        "a subgroup exactly when it is a nonempty subset closed under the "
        "operation and closed under taking inverses -- a self-contained "
        "cell that never has to phone headquarters for backup."
    ),
    "judson:6.1-cosets": (
        "Cosets is the next move in mapping Q Branch's reach. Pick any "
        "agent g in the network G: the left coset gH is the set of "
        "products gh for every h already in Q Branch, and the right coset "
        "Hg runs the same idea the other way. Every agent in G lands in "
        "exactly one left coset of Q Branch, so the cosets tile "
        "headquarters' whole roster into non-overlapping blocks -- "
        "exactly the tiling Lagrange is about to make precise."
    ),
    TARGET_NODE_ID: (
...
def build_planner_state() -> PlannerState:
    ...
    # the whole 11-node graph -- the Homomorphisms/Normal-Subgroups/
    # Isomorphisms branch (kept so math_facts' kernel-normal-subgroup fact
    # is reachable from *some* traversal) is off this lesson's path to
    # Lagrange, per judson_algebra_graph.py's own docstring ("Lagrange's
    # Theorem remains the target used by teach-8xw.15's acceptance test").
    # Covering that second fact is a different lesson, not something to
    # fold in here just to raise a coverage count -- see this module's
    # self-check and the integration report for the explicit disclosure of
    # that scope boundary.
                ...
                render_authority(
                    MotivationMoment(
                        ...
                        cited_source='Judson\'s Abstract Algebra: Theory and Applications, '
                                     'chapter "Groups", section "Subgroups"',
                        authority_fact=(
            ...
            _bond_voice(
                render_scarcity(
                    MotivationMoment(
                        concept_name="cosets",
                        scarce_detail=(
                            "the left coset gH and the right coset Hg are "
                            "built from the same subgroup but are not "
                            "generally the same set"
                        ),
                    )
                )
            )
...
def _self_check() -> None:
    ...
    liking = render_liking(
        MotivationMoment(concept_name="Sets and Equivalence Relations", shared_frame="the Bond briefing")
    )
    ...
    scarcity = render_scarcity(
        MotivationMoment(
            concept_name="cosets",
            scarce_detail=(
                "the left coset gH and the right coset Hg are built from "
                "the same subgroup but are not generally the same set"
            ),
        )
    )
    ...
    print(
        f"OK: built a {len(artifact.turns)}-turn Bond-framed Judson lesson "
